...
#endif
struct SpineMTKSwiftUIView2: Equatable {
    ...
    func makeCoordinator() -> SpineMTKViewDefaultDelegate {
        let atlas = try! SpineAtlasBox(path: atlasURL.path)
        let shared = try! SpineSkeletonDataBox(atlas: atlas, skelPath: skelURL.path)
        let drawable = TaggedDrawable(resource: .init(skeletonData: shared))
        drawable.setTag(tag)
        if let animationName = self.animationName {
            spine_support.animationState_set(&drawable.animationState, 0, animationName, true)
...
#if os(macOS)
extension SpineMTKSwiftUIView2: NSViewRepresentable {
...
#if os(iOS) || os(visionOS) || os(tvOS)
extension SpineMTKSwiftUIView2: UIViewRepresentable {
